...
class S2ConvNet_original(torch.nn.Module):
    def __init__(self):
        ...
        # spherical grid -> spherical tensor
        self.from_s2 = o3.FromS2Grid((b_in, b_in), lmax1) 
        # 1 feature on S2 -> f1 features on SO(3)
        self.conv1 = S2Convolution(1, f1, lmax1, kernel_grid=grid_s2)
        # SO3Activation(input_lmax, output_lmax, activation_function, so(3)_grid_resolution)
        self.act1 = SO3Activation(lmax1, lmax2, torch.relu, b_l1)
        # SO3Convolution(input_features, output_features, input_lmax, kernel_grid)
    ...
    def forward(self, x):
        ...
        x = self.from_s2(x)  # [batch, features, beta13., alpha] -> [batch, features, irreps]
        ...
        x = x.flatten(1) @ self.w_out / self.w_out.shape[0] # multiply with a matrix that makes it f_output dimensional, followed by normalization
...
def load_data(path, batch_size):
    ...
    for i in range(len(dataset) - 1):
        test_data = torch.from_numpy(dataset["test" + str(i)]["images"][:, None, :, :].astype(np.float32))
        test_labels = torch.from_numpy(dataset["test" + str(i)]["labels"].astype(np.int64))
        test_dataset = torch.utils.data.TensorDataset(test_data, test_labels)
        test_loader_dict[i] = torch.utils.data.DataLoader(test_dataset, batch_size=batch_size, shuffle=True)

    # test_data /= 57
    
    return train_loader, test_loader_dict, train_dataset, test_dataset


def main():
    train_loader, test_loader_dict, train_dataset, _ = load_data(MNIST_PATH, BATCH_SIZE)
    ...
    for item in test_loader_dict:
        correct = 0
        total = 0
        for images, labels in test_loader_dict[item]:
        ...
        print(str(item) + ":" + "Test Accuracy: {0}".format(100 * correct / total))
